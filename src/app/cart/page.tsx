...
export default function CartPage() {
	...
	const changeQty = (id: string, size: string | null | undefined, newQty: number, variants?: Record<string, { name_en: string; value_en: string; price: number }>) => {
		if (newQty < 1) return;
	...
	const remove = (id: string, size: string | null | undefined, variants?: Record<string, { name_en: string; value_en: string; price: number }>) => {
				...
				<div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
					...
					<div className="lg:col-span-2">
						<div className="space-y-6">
							...
							{items.map((item) => (
								<div key={`${item.id}-${item.variants ? JSON.stringify(item.variants) : "no-variants"}`} className="flex gap-4 rounded-lg border border-black/10 p-4">
									{/* Image */}
									...
									<div className="flex-1">
										...
										<div className="mt-3 flex flex-wrap items-center gap-4">
											<div className="flex items-center gap-2">
												<button onClick={() => changeQty(item.id, null, item.qty - 1, item.variants)} className="inline-flex h-8 w-8 items-center justify-center rounded border border-black/20">
