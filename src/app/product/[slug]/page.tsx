...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
		...
		const fetchProductData = async () => {
			try {
				...
				if (productData.has_variants) {
					try {
						const variantsData = await api.getProductVariants(productData.id);
						setVariants(variantsData);
						
						// Auto-select first available variant
						if (variantsData.length > 0) {
							const autoSelectedVariants: Record<string, ProductVariant> = {};
							let initialVariantPrice = productData.price;
							
							// Find the first available variant
							const firstAvailableVariant = variantsData.find(variant => 
								variant.is_active && variant.stock_quantity > 0
							);
							
							if (firstAvailableVariant) {
								// Use the product's variant type as the key
								const variantType = productData.variant_type_en || "Options";
								autoSelectedVariants[variantType] = firstAvailableVariant;
								
								// Use the variant's absolute price
								if (firstAvailableVariant.price > initialVariantPrice) {
									initialVariantPrice = firstAvailableVariant.price;
								}
							}
	...
	useEffect(() => {
		...
		if (savedFavorites) {
			try {
				...
				console.log('Initialized favorites from localStorage:', Array.from(favoritesSet));
	...
	useEffect(() => {
		...
		if (product) {
			console.log('Current product slug:', product.slug, 'Is favorited:', favorites.has(product.slug));
		}
	}, [favorites, product]);

	// Helper function to check if current product is in favorites (directly from localStorage)
	...
	const handleToggleFavorite = (productSlug: string, productData: { id: string; name: string; price: number; image?: string }) => {
		...
		setFavorites(prev => {
			...
			if (newFavorites.has(productSlug)) {
				...
				console.log(`Removed ${productSlug} from favorites`);
			} else {
				newFavorites.add(productSlug);
				console.log(`Added ${productSlug} to favorites`);
			}
	...
	const handleMainProductToggle = () => {
		if (product) {
			console.log('Toggling favorite for main product:', product.slug);
			const productData = {
				id: product.slug,
	...
	const onAddToCart = () => {
		if (product) {
			addToCart({ 
				id: product.id.toString(), // Use the actual product ID, not the slug
				slug, 
				name: product.name_en, 
				price: currentPrice, 
				qty: 1, 
				...
				variants: Object.keys(selectedVariants).length > 0 ? 
					Object.entries(selectedVariants).reduce((acc, [key, variant]) => {
						acc[key] = {
							name_en: key, // Use the variant type (e.g., "Size", "Color") as the name
					...
					<div>
						...
						<div className="mt-4">
							<div className="flex items-baseline gap-2">
								...
								{product.sale_price && product.sale_price > 0 && (
									<span className="ml-2 text-lg text-gray-500 line-through">{product.sale_price.toFixed(2)} RON</span>
								)}
							</div>
						</div>
						...
						<div className="mt-6 pt-6 border-t border-gray-200">
							...
							<div className="flex gap-3">
								<a
									href={`https://wa.me/40741302753?text=Hello I am interested in ${product.name_en}. Product URL: ${window.location.origin}/product/${product.slug}`}
									target="_blank"
